locals {
  shutter_apps = [for k, v in var.frontends : v if lookup(v, "shutter_app", false)]
}
module "static_webapp" {
  ...
  source              = "git::https://github.com/hmcts/terraform-module-shutter-static-webapp.git?ref=change_validation"
