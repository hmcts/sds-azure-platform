resource "azurerm_cdn_endpoint" "shutter_endpoint" {
  ...
  name                   = each.value.shutter_name_override != null ? substr("hmcts-${each.value.shutter_name_override}-shutter-${var.env}", 0, 48) : substr("hmcts${split(".", replace("${each.value.custom_domain}", "www.", ""))[0]}-shutter-${var.env}", 0, 48)
