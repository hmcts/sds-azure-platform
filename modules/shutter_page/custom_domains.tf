resource "azurerm_cdn_endpoint_custom_domain" "this" {
  ...
  dynamic "cdn_managed_https" {
    for_each = lookup(each.value, "ssl_mode", var.ssl_mode) == "AzureKeyVault" ? {} : { type = "Dedicated" }
    content {
  ...
  dynamic "user_managed_https" {
    for_each = lookup(each.value, "ssl_mode", var.ssl_mode) == "AzureKeyVault" ? { key_vault_certificate_id = data.azurerm_key_vault_secret.certificate[each.value["name"]].id } : {}
    content {
